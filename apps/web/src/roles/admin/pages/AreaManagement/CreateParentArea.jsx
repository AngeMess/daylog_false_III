// CreateParentArea.jsx (Updated)
import React, { useState } from 'react';
import ConfirmationModal from '../../../../components/ui/ConfirmationModal.jsx';
import { Pyramid, AlertCircle, AlertTriangle, CheckCircle, EllipsisVertical, Edit, Trash2 } from 'lucide-react';
import FormInput from '../../../../components/ui/FormInput.jsx';
import useMainAreas from '../../hooks/useMainAreaApi.js';
...
const CreateParentArea = ({ onCancel }) => {
  const [nombreAreaMadre, setNombreAreaMadre] = useState('');
  ...
  const [updateNombreAreaMadre, setUpdateNombreAreaMadre] = useState('');
  const [updateConfirmarNombre, setUpdateConfirmarNombre] = useState('');
  const [updateLoading, setUpdateLoading] = useState(false);

  // Hook para manejar las áreas madre
  const {
    mainAreas,
  ...
  // Validaciones para crear nueva área madre
  const isNombreValid = nombreAreaMadre.trim().length > 0 && nombreAreaMadre.trim().length <= 300;
  const isConfirmValid = confirmarNombre === nombreAreaMadre && isNombreValid;
  const isFormValid = isNombreValid && isConfirmValid;

  // Validaciones para actualizar área madre
  const isUpdateNombreValid = updateNombreAreaMadre.trim().length > 0 && updateNombreAreaMadre.trim().length <= 300;
  const isUpdateConfirmValid = updateConfirmarNombre === updateNombreAreaMadre && isUpdateNombreValid;
  const isUpdateFormValid = isUpdateNombreValid && isUpdateConfirmValid;

  // Combinar loading states: local + hook
  const isLoading = localLoading || mainAreasLoading;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const mainAreaData = {
        name: nombreAreaMadre.trim()
      };

      await createMainArea(mainAreaData);

      showAlert('success', 'Área madre creada correctamente');
      ...
      console.error('Error al crear área madre:', error);
      showAlert('error', error.message || 'Error al crear el área madre');
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMainArea(areaToDelete._id);
      setShowDeleteModal(false);
      setAreaToDelete(null);
      showAlert('success', 'Área madre eliminada correctamente');
    } catch (error) {
      console.error('Error al eliminar área madre:', error);
      showAlert('error', error.message || 'Error al eliminar el área madre');
    } finally {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const updateData = {
        name: updateNombreAreaMadre.trim()
      };

      await updateMainArea(areaToUpdate._id, updateData);

      setShowUpdateModal(false);
      setAreaToUpdate(null);
      setUpdateNombreAreaMadre('');
      setUpdateConfirmarNombre('');

      showAlert('success', 'Área madre actualizada correctamente');

    } catch (error) {
      console.error('Error al actualizar área madre:', error);
      showAlert('error', error.message || 'Error al actualizar el área madre');
    } finally {
      setUpdateLoading(false);
    }
  };

  const openUpdateModal = (mainArea) => {
    setAreaToUpdate(mainArea);
    setUpdateNombreAreaMadre(mainArea.name);
    setUpdateConfirmarNombre(mainArea.name);
  ...
  // Función para renderizar el contenido de las áreas madre
  const renderMainAreasContent = () => {
    ...
    if (error) {
      return (
        <ErrorState
          message={error.message || "Error al cargar las áreas madre"}
          onRetry={() => {
            clearError();
            refetch();
          }}
    ...
    if (!mainAreas || mainAreas.length === 0) {
      return (
        <EmptyState
          message="No hay áreas madre registradas"
          description="Crea tu primera área madre usando el formulario de arriba"
          icon={Pyramid}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
        {mainAreas.map((mainArea) => (
          <GenericAreaCard // Usar el nuevo componente
            ...
            isParent={true} // Indicate that this is a parent area
          />
        ))}
      </div>
    );
    ...
    <div className="p-6">
      <div className="flex flex-col items-start mb-10">
        ...
        <div className="w-full">
          <form onSubmit={handleSubmit} className="mb-8">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-3/4 md:pr-12">
                {/* Campo: Nombre del área madre */}
                <FormInput
                  id="nombreAreaMadre"
                  label="Nombre de la nueva área madre"
                  value={nombreAreaMadre}
                  onChange={(e) => setNombreAreaMadre(e.target.value)}
                  icon={Pyramid}
                  required
                  className="mb-4"
                />
                ...
                {nombreAreaMadre.length > 0 && (
                  <p className={`mt-1 mb-2 text-sm ${nombreAreaMadre.length > 300 ? 'text-red-600' : 'text-gray-500'}`}>
                    {nombreAreaMadre.length}/300 caracteres
                  </p>
                )}
                ...
                <FormInput
                  id="confirmarNombre"
                  label="Confirmar nombre de área madre"
          ...
          {/* Sección de áreas madre usando StateHandler */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative z-10">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              ...
              Actualizar área madre
            </h2>

            <form onSubmit={handleUpdateSubmit}>
              {/* Nombre del área madre */}
              <FormInput
                id="updateNombreAreaMadre"
                label="Nombre del área madre"
                value={updateNombreAreaMadre}
                onChange={(e) => setUpdateNombreAreaMadre(e.target.value)}
                icon={Pyramid}
                required
                className="mb-4"
              />

              {updateNombreAreaMadre.length > 0 && (
                <p className={`mt-1 mb-2 text-sm ${updateNombreAreaMadre.length > 300 ? 'text-red-600' : 'text-gray-500'}`}>
                  {updateNombreAreaMadre.length}/300 caracteres
                </p>
              )}
...
export default CreateParentArea;