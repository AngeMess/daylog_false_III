...
const useWorkTeam = () => {
  ...
  const searchEmployees = async (searchTerm = '', roleFilter = null) => {
    try {
      ...
      let activeEmployees = allEmployees.filter(employee => employee.isActive);

      let filteredByRole = activeEmployees;
      if (roleFilter) {
        filteredByRole = activeEmployees.filter(employee =>
  ...
  const searchMultipleRoleEmployees = async (searchTerm = '') => {
    try {
      ...
      let filteredByRole = activeEmployees.filter(employee =>
  ...
  const getEmployeesByRole = async (role) => {
    try {
      ...
      if (!response.ok) throw new Error('Error al obtener empleados');
      const allEmployees = await response.json();
      // Filter out inactive employees first, then by role
      return allEmployees.filter(employee => employee.isActive && employee.daylogRol === role);
    } catch (err) {
      setError(err.message);
      return [];
    }
  ...
  const getEmployeesByMultipleRoles = async (roles) => {
    try {
      ...
      if (!response.ok) throw new Error('Error al obtener empleados');
      const allEmployees = await response.json();
      // Filter out inactive employees first, then by multiple roles
      return allEmployees.filter(employee => employee.isActive && roles.includes(employee.daylogRol));
    } catch (err) {
      setError(err.message);
      return [];
    }
...
const updateWorkTeam = async (id, workTeamData) => {
  ...
  const getWorkTeamsByEmployee = useCallback(async (employeeId) => { 
    try {
      setLoading(true);
      setError(null);
