...
/**
 * Hook personalizado para manejar la lógica de la página de detalles de proyecto en portfolio
 * 
 ...
 * - Adaptar la lógica específica para el rol de portfolio
 * 
...
export const useProjectDetail = (projectId) => {
  ...
  useEffect(() => {
    ...
    const fetchProjectData = async () => {
      try {
        console.log('🔄 Hook: Iniciando carga de datos para proyecto:', projectId);
        
        // Paso 1: Obtener datos completos del proyecto con populate
        const projectData = await getProyectById(projectId);
        console.log('📋 Hook: Datos del proyecto obtenidos:', projectData);
        
        if (projectData) {
          setProject(projectData);
          
          // Paso 2: Obtener conteo de actividades por empleado
          console.log('📊 Hook: Obteniendo conteo de actividades...');
          const activities = await getEmployeeActivityCounts(projectId);
          console.log('📊 Hook: Conteo de actividades obtenido:', activities);
          ...
          if (projectData.workTeam && Array.isArray(projectData.workTeam.employees)) {
            console.log('👥 Hook: Procesando empleados del workTeam:', projectData.workTeam.employees.length);
            ...
            console.log('🗺️ Hook: Mapa de conteos creado:', countMap);
            ...
            employeeList = projectData.workTeam.employees
              ...
              .map((emp) => {
                ...
                console.log('👤 Hook: Empleado procesado:', employeeData);
                return employeeData;
              });
          } else {
            console.warn('⚠️ Hook: No se encontró workTeam o empleados en el proyecto');
          }

          console.log('✅ Hook: Lista final de empleados:', employeeList);
          setEmployees(employeeList);
        } else {
          console.error('❌ Hook: No se obtuvieron datos del proyecto');
        }
      } catch (err) {
        console.error('❌ Hook: Error al cargar datos del proyecto:', err);
      }
    };
